...
def rename_images(input_directory):
    ...
    for base_name, file_names in tqdm(base_names.items(), desc="Renaming images", unit="image"):
        if len(file_names) == 2:
            ...
            with open(os.path.join(input_directory, text_file), 'r') as f:
            ...
            os.rename(os.path.join(input_directory, image_file), os.path.join(input_directory, new_image_name_with_counter))
